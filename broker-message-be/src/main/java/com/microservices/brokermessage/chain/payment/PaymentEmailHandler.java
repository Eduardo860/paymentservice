 ...
 */
@Component
public class PaymentEmailHandler extends AbstractRetryHandler {
    ...
    public void handle(RetryContext context) {
        logger.info("PaymentEmailHandler: sending notification for job={}", context.getJobId());
        try {
            String subject = "[Payment Retry] " + context.getAction() + " SUCCESS — entity: " + context.getEntityId();
            String body = String.format(
                "Payment retry job completed successfully.%n%n" +
                "Job ID  : %s%n" +
                "Entity  : %s%n" +
                "Action  : %s%n%n" +
                "Result  : %s",
            ...
            logger.info("PaymentEmailHandler: email sent for job={}", context.getJobId());

        } catch (Exception e) {
            logger.warn("PaymentEmailHandler: email failed for job={} — {}", context.getJobId(), e.getMessage());
